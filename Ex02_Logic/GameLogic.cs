...
namespace Ex02_Logic
{
    public class GameLogic
    {
        ...
        public GameLogic(string i_Player1Name, string i_Player2Name, int i_BoardSize, bool i_IsBotPlay)
        {
            ...
            m_Player2 = new Player((i_IsBotPlay == true) ? "Computer" : i_Player2Name);
        ...
        {
            if (m_CurrentTurn == eTurn.Player2 && m_IsBotPlay == true)
            {
                ...
                for (int i = 0; i < m_Board.BoardSize; i++)
                {
                    for (int j = 0; j < m_Board.BoardSize; j++)
                    {
                        if (m_Board.GetCell(i, j).IsCellEmpty() == true)
                        {
                            ...
                            int score = miniMax(m_Board, 0, opponentSymbol, alpha, beta);
        ...
        private int miniMax(GameBoard i_Board, int i_Depth, eCell i_MaximizingPlayer, int i_Alpha, int i_Beta)
        {
            ...
            for (int i = 0; i < boardSize; i++)
            {
                for (int j = 0; j < boardSize; j++)
                {
                    if (i_Board.GetCell(i, j).Cell == eCell.EmptyCell)
                    {
                        ...
                        int value = miniMax(i_Board, i_Depth - 1, i_MaximizingPlayer == eCell.X ? eCell.O : eCell.X, i_Alpha, i_Beta);
